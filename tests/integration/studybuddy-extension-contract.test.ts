 ...
 *   - factory(stubPi) 调用后 registerTool 被调用 34 次（S1 6 + S2 6 + S3 3 + S4 4 + S5 2 + S6 3 + S7 2 + TTS 3 + 备份恢复 5）
 *   - 多次调用 factory 安全（每次注册 34 个工具，无异常）
...
describe("T-M1-001~004 + T-M2-001~005 studybuddy-extension × pi 底座契约对接（S1+S2+S3+S4+S5+S6+S7+TTS+备份恢复 工具装配）", () => {
  ...
  it("factory(stubPi) 调用后 registerTool 被调用 34 次（S1 6 + S2 6 + S3 3 + S4 4 + S5 2 + S6 3 + S7 2 + TTS 3 + 备份恢复 5 工具装配）", async () => {
    const factory: ExtensionFactory = createStudyBuddyExtension();
  ...
  it("多次调用 factory 安全（每次注册 34 个工具，无异常）", async () => {
    const factory: ExtensionFactory = createStudyBuddyExtension();
    const { calls, pi } = createStubPi();
    await factory(pi);
    await factory(pi);
    await factory(pi);
    expect(calls.registerTool).toBe(102);
  });
