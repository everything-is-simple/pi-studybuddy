...
**版本**：v0.1.143
**日期**：2026-08-15
**状态**：✅ T-M5-011 执行中。先在当前开发机按真实用户路径跑通，再复制同一套运行资源和默认配置到使用机；系统集成三份权威文件见 `docs/系统集成/`（总览、问题总账、验收与交付路径）。（v0.1.143：T-M5-010「设置、投递、学期备份与数据根迁移闭环」已完成 Git 收口；T-M5-005 仍 blocked，仅等待其既定三条干净 Windows 原生 UAT；T-M5-006~008 不启动，当前工作区不读取、不暂存安装器。）
...
| [系统集成](./docs/系统集成/) | ✅ T-M5-011 执行中 | 后续系统集成唯一依据：01 客观认知、02 问题总账、03 验收与交付路径。 |
