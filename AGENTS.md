...
| 文档 | 版本 | 权威范围 |
|---|---|---|
| [00-文档索引](./docs/00-文档索引-Index.md) | v0.1.97 | 文档导航 + 门禁 + 版本历史 |
...
| [04-Todo](./docs/04-任务清单-Todo-List.md) | v0.1.93 | 任务登记 + 组件治理看板 + 里程碑 M0-M4 + §6.0 M0 完成说明（v0.2.3 打包能力常态化）+ §6.3 M1 退出门槛全通过 + §6.4 M2 退出门槛全通过 + §6.5 M3 退出门槛全通过 + §6.6 M4 退出门槛（后端断裂修复 + 设置页/学期切换 + S1-S7 接线 + 打包冒烟）+ §7.6.1 M4 任务登记表（T-M4-001~009/T-M4-022 done；T-M4-010 in_progress，T-M4-011~021 pending）+ §7.5 全局执行顺序表（M4 运行时阻塞 T-M4-022 位于 24，T-M4-006~021 顺延至 25-40） |
...
| `.plan/` | ✅ 已就绪 | 任务计划目录（T-M4-009 已完成并保留计划原件；T-M4-010 本地实施完成、Git 待授权；当前唯一执行计划为 `.plan/T-M4-010-s1-home-rpc.md`） |
