...
| 文档 | 版本 | 权威范围 |
|---|---|---|
| [00-文档索引](./docs/00-文档索引-Index.md) | v0.1.102 | 文档导航 + 门禁 + 版本历史 |
...
| [04-Todo](./docs/04-任务清单-Todo-List.md) | v0.1.98 | 任务登记 + 组件治理看板 + 里程碑 M0-M4 + §6.0 M0 完成说明（v0.2.3 打包能力常态化）+ §6.3 M1 退出门槛全通过 + §6.4 M2 退出门槛全通过 + §6.5 M3 退出门槛全通过 + §6.6 M4 退出门槛（后端断裂修复 + 设置页/学期切换 + S1-S7 接线 + 打包冒烟）+ §7.6.1 M4 任务登记表（T-M4-001~010/T-M4-022 done；T-M4-023 in_progress；T-M4-011~021 pending）+ §7.5 全局执行顺序表（既有 M4 业务顺序不变） |
| [05-ERD](./docs/05-数据模型-ERD-Data-Model.md) | v0.1.2 | 全局库 + 学期库 + 三层记忆 |
| [06-API](./docs/06-API契约-API-Contracts.md) | v0.1.6 | RPC 契约 + 100+ 方法 + 9 Streams + §4 AgentEvent payload 结构化 + modelsConfig.get/set |
...
| `.plan/` | ✅ 已就绪 | 任务计划目录（T-M4-009、T-M4-010 已完成并保留计划原件作为历史范围与验收证据；当前唯一执行中任务为 T-M4-023，计划原件保留） |
| `.record/` | ✅ 已就绪 | 实施记录目录（历史记录已就绪；最新记录为 `.record/T-M4-023-实施记录.md`（审计恢复记录；T-M4-023 Git 收口待授权）） |
