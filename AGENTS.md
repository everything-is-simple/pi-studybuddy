...
**版本**：v0.1.155
**日期**：2026-08-17
**状态**：✅ M5 进行中（v0.1.154：T-M5-011 方案 B 测试夹具型 x64 setup 已生成并完成包内容扫描、win-unpacked 双启动、隔离静默安装及安装后双启动 smoke；T-M5-011 仍为唯一 in_progress，七类原生 UAT/异常恢复/双审查未完成。T-M5-006 blocked；T-M5-007/008 仍按原顺序 pending；现有 `nul` 与 `omp-windows-*.exe` 未读取、运行、暂存或删除。）
...
### 3.1 设计文档（docs/00-09 与 13 均 ✅ 已审查批准）

| 文档 | 版本 | 权威范围 |
|---|---|---|
| [00-文档索引](./docs/00-文档索引-Index.md) | v0.1.211 | 文档导航 + 门禁 + M5 剩余顺序；用户已批准 T-M5-011 为唯一 in_progress，并明确授权当前分支生成方案 B 测试夹具型 setup；T-M5-006 blocked，T-M5-007/008 仍为最终发布顺序，T-M5-005 遗留 UAT并入最终验收。 |
...
| [04-Todo](./docs/04-任务清单-Todo-List.md) | v0.1.211 | 任务登记 + M5 剩余顺序：T-M5-011 配置/七类设置为唯一 in_progress，并在用户明确授权下生成测试夹具型 setup；T-M5-006 blocked，随后才是发布前真实 Electron UAT与最终候选/干净机安装验收；T-M5-005 遗留 UAT并入最终验收。 |
...
| [08-Test](./docs/08-测试验收-Test-Plan.md) | v0.1.7 | 测试金字塔 + 四层分层 + 安全不变量 + 专用测试数据库边界 + 已登记唯一隔离 E2E `test.turnEndIndex` 桥接例外 + §6 E2E 框架改 vitest + Electron + §6.6 用户端到端测试铁律（真机 UAT）+ §4.2 model_select 断言落点业务数据根 |
| [09-UI](./docs/09-使用者介面-UI-Design.md) | v0.1.5 | 三栏布局 + 💬 对话默认 Tab + S1-S7 标签页 + 模型选择持久化业务数据根 |
| [13-测试与运维](./docs/13-测试与运维-Testing-and-Operations.md) | ✅ v0.1.3 已审查批准 | 用户动作、错误责任、数据资产/SQLite 生命周期、真实测试、真机 UAT、发布与运维门禁；T-M5-009 已完成首次追溯基线，T-M5-005 正在按范围消费。 |
...
| `.plan/` | ✅ 已就绪 | 任务计划目录；当前唯一执行计划为 `.plan/T-M5-011-local-config-settings-console.md`，T-M5-006 计划保留为 blocked 的历史与审查证据 |
...
| 版本 | 日期 | 变更 |
| v0.1.155 | 2026-08-17 | 用户明确要求将方案 B 测试安装包重新命名为 `pi-studybuddy-test--方案b-setup包.exe`。已同步安装器文件名、Windows 产品名（`Pi StudyBuddy 方案b-setup包`）、`test-bundle` manifest displayName 与启动器标题；新包已重新生成并验证，SHA-256 `09c5ed2a27b8e4521596e5bc8fe7e3765bfe9fdadfc04e0c40aa71b5f87d6a98`，130418209 bytes，win-unpacked 带 test profile 的两次隔离启动、`global.db`、`system.ping` 与业务 RPC 通过，隔离根生成了合成学期/课程/资料/知识模块/考试且无 `credentials.json`。旧包 `pi-studybuddy-test-x64-setup.exe`（SHA-256 `4a52a7bf...`）被新命名取代，哈希作为历史证据保留。T-M5-011 仍 in_progress，不替代七类原生 UAT、异常恢复、双审查或 T-M5-007/008。 |
| v0.1.154 | 2026-08-17 | 方案 B 测试 setup 实施证据回填：新增正式 handler 驱动的显式 test profile、`test-bundle`、独立 electron-builder 配置、包内容扫描与 package smoke；Node24 全量 unit/integration 143 files/1256 tests、type-check、contract 132/132、安全 6/6、文档治理和 diff-check 通过。`release-test/pi-studybuddy-test-x64-setup.exe` 已生成，SHA-256 `4a52a7bf2729b3ade6e5eaf15e6f1c7ef03f03814a188c3d5a3b5a840f8f68ae`，130417990 bytes；win-unpacked 双启动通过，隔离静默安装最终完成，安装后无残留进程复验双启动通过。首次安装扫描和首次安装后 smoke 的超时均保留为环境证据，不掩盖。任务仍 in_progress，不替代七类原生 UAT、异常恢复、双审查或 T-M5-007/008。 |
| v0.1.153 | 2026-08-17 | 用户明确确认方案 B 测试夹具型 setup 生成。该授权是当前 T-M5-011 分支的受控测试资产例外：允许生成新的隔离 x64 setup 与 `test-bundle`，只含合成业务 fixture、占位配置、显式 test profile 启动器、受控 mock 运行标识和已审计合法运行资源；禁止读取/运行/暂存现有安装器，禁止改变 T-M5-007 发布前 UAT与 T-M5-008 最终候选的顺序和验收含义。原因：用户明确授权；影响：当前计划新增测试 setup 施工与包内容扫描，不改变生产数据、凭据边界或完成判据。依据：用户明确指令 + AGENTS.md §2/§4.4/§4.5/§5/§7/§9/§11。 |
...
| v0.1.148 | 2026-08-17 | 同步 T-M5-006 的最终本地验证事实：Node24 `verify --stage=full --skip=e2e` 已执行 8 项、明确跳过历史安装器 E2E；串行 `pnpm test` 为 139 files/1240 tests，无发行真实 Electron 回归为 33 files/141 tests，当前 `win-unpacked` package smoke 两次隔离启动通过。一次与大量 Electron/文件系统任务并发时出现的 file-watch/package-smoke 抖动，已在无残留进程下单独与串行复验通过，保留为环境证据而不掩盖。影响仅当前任务证据、00/04 导航版本与收尾门禁状态，不改 API/schema/任务顺序/安装器；任务仍 in_progress，仍待双独立审查与用户授权收尾。依据：用户“继续”授权当前任务 + §2/§4.4/§5/§7/§9/§11。 |
| v0.1.147 | 2026-08-17 | 修复治理质量门 `scripts/verify.mjs` 的 skip 标签归一化：此前 `--skip=e2e` 会错误移除数字而匹配为 `ee`，导致未跳过历史 T-M4-021 安装器验收；现在命令行参数与检查标签均保留数字并使用同一规范化函数，已用仅文档治理的 full-stage 命令复验 E2E 确实跳过。原因：T-M5-006 禁止在 T-M5-008 前生成候选，且质量门参数必须可信。影响仅质量门选择逻辑和当前任务证据，不改业务 API/schema/handler、任务顺序或安装器；已如实保留先前误触发事实。依据：用户“继续”授权当前 T-M5-006 + §2/§4.4/§5/§7/§9/§11。 |
...
| v0.1.141 | 2026-08-15 | 用户裁决已发现的设置页产品缺口必须完整修复，不以既定干净机 UAT 替代实现。T-M5-005 由 in_progress 改为 blocked，仅等待三条已定义原生 UAT；新增并启动 T-M5-010「设置、投递、学期备份与数据根迁移闭环」作为唯一执行任务，允许新增所需 API/schema/handler/迁移协议和隔离测试。T-M5-006~008 仍不启动，安装器不读取、不暂存。原因：用户明确指令；影响：任务顺序与实施范围扩展，不宣称 T-M5-005 已完成。依据：用户裁决 + §2/§4.4/§4.5/§5/§7/§9/§11。|
| v0.1.142 | 2026-08-15 | 用户明确授权 T-M5-010 执行 commit、任务分支 push、master ff-only 合并、master push 与远端同位核验；T-M5-010 完成实现/隔离 UAT/独立审查/Node24 完整质量门后进入 Git 收口，T-M5-005 继续 blocked，T-M5-006~008 不启动，安装器不读取、不暂存。依据：用户明确指令 + AGENTS.md §4.5/§7/§8.2/§8.4/§11。 |
...
| v0.1.134 | 2026-08-14 | T-M5-005 RED→GREEN 与部分原生 UAT事实同步：补足 S5 速背卡、S6 报告、S7 转写结果朗读入口并复用 AppShell TTS 所有权，`overwrite` 恢复先确认；定向 renderer 4/4、相关 RPC 47/47、Electron renderer E2E 7/7、Node24 type-check/build 通过。隔离根 UAT 已证实学期/课程创建与重启回读、设置保存反馈、采集合规空态和 WAV 选择器；S6 完整报告/投递、S7 成功转写、真实恢复、设置值重启回读和 TTS 已复习持久化仍未覆盖。任务保持 in_progress，不启动 T-M5-006~008。依据：用户开工授权 + §5/§7/§11。 |
| v0.1.133 | 2026-08-14 | 用户明确批准 T-M5-005「S6/S7/TTS/备份/设置与整体 UX 修订」开工：04-Todo pending→in_progress，建立唯一计划 `.plan/T-M5-005-s6-s7-tts-backup-settings-ux.md` 与隔离分支 `agent/T-M5-005-s6-s7-tts-backup-settings-ux`；先完成源码/测试事实盘点并写 RED，消费 T-M5-009 的追溯基线。范围不含 T-M5-006 依赖自包含、T-M5-007 干净机 UAT、T-M5-008 最终发行，不写生产数据，不启动后续任务，Git 收口另待授权。依据：用户明确授权 + §4.4/§4.5/§5/§7/§8/§11。 |
| v0.1.132 | 2026-08-14 | 同步 T-M5-009 受控 Git 收口最终事实：功能/基线提交 `156756c` 已 ff-only 合并 master，Node24 master `verify --stage=full` 最终重跑通过（131 files/1195 tests、真实 Electron E2E 33 files/141 tests），治理登记已推送且 `master=origin/master` 同位核验完成，任务 in_progress→done。用户认可 `T-M5-009-DOC-CONSUMER-001` PASS 仅替代该无 Electron UI 变更任务的真机 UAT，例外不外推；T-M5-005~008 保持 pending。原因：修复当前状态滞后；影响：仅治理事实同步，不改历史记录、业务代码、API/schema/handler 或数据。依据：用户明确批准 + §2、§4.5、§7、§8.4、§11。 |
| v0.1.129 | 2026-08-14 | T-M5-009 RED→GREEN 已完成：建立 `docs/traceability/` 六项追溯/运维资产、任务级集成测试与实施记录；Node24 `verify --stage=full`、文档治理、diff-check 通过。任务保持 in_progress，真机 UAT 与 Git 收口仍待单独门禁；不启动 T-M5-005~008。依据：用户开工授权 + §4.4/§4.5/§5/§7/§8/§11。 |
| v0.1.128 | 2026-08-14 | 用户明确批准 T-M5-009 开工：建立唯一计划 `.plan/T-M5-009-test-operations-traceability-baseline.md`、切换分支 `agent/T-M5-009-traceability-baseline`，04-Todo pending→in_progress；范围仅治理盘点与追溯基线，不新增业务 API/schema/handler/入口、不写生产数据、不启动 T-M5-005~008，Git 收口另待授权。依据：用户明确授权 + §4.4/§4.5/§5/§7/§8/§11。 |
| v0.1.127 | 2026-08-14 | 用户审查批准 docs/13 v0.1.1，并按方案 B 新增 T-M5-009「测试与运维追溯基线建立」为 pending；同步 00-索引 v0.1.177、04-Todo v0.1.179 和当前任务看板，且修正 04-Todo 的 T-M5-004 陈旧 in_progress 状态为 done。影响：只批准规范/登记任务，不创建 T-M5-009 详细计划或分支、不改业务 API/schema/handler/入口、不写生产数据、不执行 Git 收口。依据：用户本次明确授权 + §2、§4.4、§7、§8、§11。 |
| v0.1.126 | 2026-08-14 | T-M5-004 正式完成登记：`47639f9`（功能）已快进合并 master；`715add3`（本地收口登记）已成功推送并核验 `master=origin/master=715add3`。本最终完成登记提交随后的 `git push origin master` 与远端同位核验构成 §8.4 最后一步；任务由 in_progress→done，M5 统计为 4 pending/0 in_progress/4 done（总计 4/0/66）。未启动 T-M5-005~008。依据：用户授权 + §7/§8.2/§8.4/§11。 |
| v0.1.125 | 2026-08-14 | 用户明确授权 T-M5-004 Git 收口。功能提交 `47639f9` 已由 `agent/T-M5-004-test-database-real-data` 快进合并至 `master`；Node24.14.0/pnpm11.20.0 的 master `pnpm verify --stage=full` 通过（unit/integration 130 files/1192 tests、真实 Electron E2E 33 files/141 tests、执行 8/跳过 2、exit 0）。本次登记仅记录远端推送前的本地收口事实；按 §8.4，任务在 `origin/master` 推送与同位核验前仍为 in_progress。影响：授权状态、Git 事实与版本同步，不启动 T-M5-005~008。依据：用户明确授权 + §7/§8.2/§8.4/§11。 |
| v0.1.124 | 2026-08-14 | 同步 T-M5-004 非 Git 收尾准备：v4c 原生 UAT 的 S3 第二次重启可见结果读取、Node24 完整质量门（130/1192，真实 Electron E2E 33/141）及独立审查 A/B 最终 P0/P1=0 已登记；任务保持 in_progress，仍未获 Git 收口授权。影响：治理状态与文档版本同步，不宣称 done、不改范围/API/schema/handler 或 Git 权限。依据：用户 UAT 铁律 + §2/§7/§8.4/§11。 |
| v0.1.123 | 2026-08-14 | 修复 v0.1.119 与 v0.1.118 修订记录之间误写入的转义换行文本，恢复为真实 Markdown 表格换行。原因：T-M5-004 复审 B 发现该残留将两条治理历史粘连，影响可读性与审计。影响：仅修订记录格式；不改变历史事实、任务状态、范围、API/schema/handler 或 Git 权限。依据：复审 B + §2/§11.1/§11.2。 |

| v0.1.119 | 2026-08-13 | 用户明确裁决 T-M5-004 的 S2/S3/S4 创建前置条件属于本任务；原生真机 UAT 必须补齐 S1 创建任务→完成→重启回读、S2 创建资料→使用→重启回读、S3 UI 创建前置→提交练习→重启回读、S4 真实错答创建→重做→重启回读。禁止 renderer 注入/CDP/RPC-handler 直调/数据库预置替代；任务保持 in_progress，Git 收口仍须用户单独授权。依据：用户明确指令 + §2/§5/§7/§11。 |
| v0.1.118 | 2026-08-13 | 新增 `docs/13-测试与运维-Testing-and-Operations.md` v0.1.0 草案，并在 00-文档索引、README 与 §3.1 同步登记。文档定义用户动作目录、错误责任、数据资产/SQLite 生命周期、测试真实性、真机 UAT、发布与运维门禁；其状态为待用户审查，不自动改变 T-M5-004 的 in_progress、任务范围或 Git 收口资格。原因：用户明确要求将测试阶段治理 prompt 演化为 docs 第 13 份文档。影响：新增治理追溯标准，不改 API/handler/schema/生产数据/任务状态。依据：用户明确指令 + AGENTS.md §2、§11.1、§11.2。 |
| v0.1.117 | 2026-08-13 | 测试阶段数据库修正：用户明确要求系统测试不依赖 mock 业务数据、建立专门测试数据库。确认生产数据为业务根 `%LOCALAPPDATA%\PiStudyBuddy\global.db` + `semester/<id>/sem.db`；测试采用 `PI_STUDYBUDDY_DATA_ROOT` 指向 `H:\pi-studybuddy-tmp\runs\<task-id>`。新增专用测试数据库构建器，E2E-04 启动真实 Electron 前以正式 schema/handler 建立 S1/S2/S5 前置数据并移除运行中 `test.seedModule` RPC；`initializeDataRoot()` 建库后关闭 SQLite 连接，修复 Windows WAL/SHM 锁。Node24 `verify --stage=full` 最终复验通过（unit/integration 130 files/1183 tests、真实 Electron E2E 33 files/141 tests）。T-M5-004 保持 in_progress：renderer 自动化不得充当原生真机 UAT，S1 完整闭环与 S2/S3/S4 完整闭环范围裁决未完成；未 commit/merge/push。依据：用户明确指令 + AGENTS.md §5.3/§7/§8.4/§9.5/§11.1/§11.2。 |
| v0.1.116 | 2026-08-13 | 独立复验修正：独立审查者发现 v0.1.115 的 T-M5-004 done 登记过早——UAT v1 仅验证 S1 创建 + S2-S5 空态/可达性，未走「创建→使用→重启回查」完整闭环（不满足 §6.6 铁律）；截图仅 01 步 PNG；全量 verify 曾因 T-M4-021 残留进程锁 + T-M5-003 截图 0 字节非全绿。处置：T-M5-004 done→in_progress 回退；UAT v2 当时登记为完整闭环（后续 v0.1.117 复查降级为 renderer 自动化，非原生真机 UAT）；修复 2 个 S5 空课程模拟考 FK 生产缺陷（mock-exam-generator.ts 空模块 default-module 假引用→null；mock-exams.ts submitAttempt 写 mock_exam_module_analyses 时 null/unknown 模块→空课程跳过分析写入）；新增 GEN-05 测试；全量 verify full 全绿（清理残留进程后：unit/integration 128 files/1175 tests、真实 Electron E2E 33 files/142 tests、contract 127/127、安全 6/6、UUID 7/7、docs 治理与 diff-check；T-M4-021 单独稳定化通过，残留进程环境性非回归）；docs/04 v0.1.170、实施记录附复验修正章；Git 收口待用户再次授权，任务保持 in_progress。依据：独立复验结论 + AGENTS.md §5/§7/§8.4/§11.1/§11.2。 |
| v0.1.115 | 2026-08-13 | T-M5-004 曾登记 Git 收口完成（后续 v0.1.116/v0.1.117 supersede）：功能提交 `c4bb784`（feat(m5) S1-S5 逐控件真实闭环）与治理登记提交 `5eb4e67`（docs(m5) 本地实施与验收登记）已由 `agent/T-M5-004-s1-s5-structured-learning-ui` 快进合并进入 `master` 并推送 origin/master，核验 `master=origin/master=5eb4e67`（经 http.curloptResolve 备用 IP 通道 20.27.177.113，一次性，不落盘）。交付：用户裁决 CTRL-MISTAKE-04 方案 A（mistakes.get 返回 question 摘要，handler 既有数据，可选字段向后兼容，contract 方法数不变 127/127）+ mistakes.redo correct? 既有能力补登；首页任务完成动作+加载失败重试、资料预览（files.previewMarkdown/read storageKey 相对路径）+retryAiGeneration、笔记思维导图+证据回链可点击、练习结果页加入错题、错题重做正确/错误双动作+详情失败重试+作答历史+完整复盘、冲刺静态按钮禁用+未确认考试拦截固定错误+生成失败恢复；移除/禁用静态无 action 按钮；真机 UAT 8 路径两阶段纯 UI 证据落 runs/T-M5-004/uat/（不进 Git）；Node24 verify --stage=full 通过（unit/integration 128 files/1174 tests、真实 Electron E2E 33 files/143 tests、contract 127/127、安全 6/6、smoke 6/6、UUID 7/7、docs 治理与 diff-check；t-m4-021 打包冒烟 ECONNRESET / t-m5-002 first-run 抖动 / t-m5-003 uat PNG 0 字节三处环境性抖动单独重跑均通过）；双独立审查 PASS；§3.1 同步 00-索引 v0.1.166/04-Todo v0.1.168；§9 统计 M5 4 pending/0 in_progress/4 done（合计 4/0/66）；不启动 T-M5-005~008。依据：用户明确 Git 收口授权 + AGENTS.md §4.5/§7/§8.2/§8.4/§11.1/§11.2。 |
