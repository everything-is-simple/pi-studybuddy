...
| 文档 | 版本 | 权威范围 |
|---|---|---|
| [00-文档索引](./docs/00-文档索引-Index.md) | v0.1.94 | 文档导航 + 门禁 + 版本历史 |
...
| [04-Todo](./docs/04-任务清单-Todo-List.md) | v0.1.88 | 任务登记 + 组件治理看板 + 里程碑 M0-M4 + §6.0 M0 完成说明（v0.2.3 打包能力常态化）+ §6.3 M1 退出门槛全通过 + §6.4 M2 退出门槛全通过 + §6.5 M3 退出门槛全通过 + §6.6 M4 退出门槛（后端断裂修复 + 设置页/学期切换 + S1-S7 接线 + 打包冒烟）+ §7.6.1 M4 任务登记表（T-M4-001~007/T-M4-022 done；T-M4-008~021 pending）+ §7.5 全局执行顺序表（M4 运行时阻塞 T-M4-022 位于 24，T-M4-006~021 顺延至 25-40） |
