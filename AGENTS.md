...
| 文档 | 版本 | 权威范围 |
|---|---|---|
| [00-文档索引](./docs/00-文档索引-Index.md) | v0.1.98 | 文档导航 + 门禁 + 版本历史 |
...
| [04-Todo](./docs/04-任务清单-Todo-List.md) | v0.1.94 | 任务登记 + 组件治理看板 + 里程碑 M0-M4 + §6.0 M0 完成说明（v0.2.3 打包能力常态化）+ §6.3 M1 退出门槛全通过 + §6.4 M2 退出门槛全通过 + §6.5 M3 退出门槛全通过 + §6.6 M4 退出门槛（后端断裂修复 + 设置页/学期切换 + S1-S7 接线 + 打包冒烟）+ §7.6.1 M4 任务登记表（T-M4-001~010/T-M4-022 done；T-M4-011~021 pending）+ §7.5 全局执行顺序表（M4 运行时阻塞 T-M4-022 位于 24，T-M4-006~021 顺延至 25-40） |
...
| `.plan/` | ✅ 已就绪 | 任务计划目录（T-M4-009、T-M4-010 均已完成并保留计划原件；当前无执行中任务） |
| `.record/` | ✅ 已就绪 | 实施记录目录（历史记录已就绪；最新记录为 `.record/T-M4-009-实施记录.md`） |
...
| v0.1.77 | 2026-08-09 | T-M4-010 Git 收口完成：功能提交 `a06d8a5` 已由 `agent/T-M4-010-s1-home-rpc` 快进合并进入 `master`；Node 24.14.0 下 master 完整 `pnpm verify -- --stage=full`、contract 127/127、安全 6/6、UUID 7/7、文档治理与 `git diff --check` 均通过；任务状态 in_progress→done，M4 统计更新为 9 pending / 0 in_progress / 11 done，总计 9 pending / 0 in_progress / 47 done；不启动 T-M4-011~021。依据：AGENTS.md §4.5、§7、§8.2、§8.4、§11.2 + 用户明确授权。 |
