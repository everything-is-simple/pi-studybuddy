...
| 文档 | 版本 | 权威范围 |
|---|---|---|
| [00-文档索引](./docs/00-文档索引-Index.md) | v0.1.91 | 文档导航 + 门禁 + 版本历史 |
| [01-TRD](./docs/01-TRD-技术需求-Technical-Requirements.md) | v0.2.3 | 技术底座决策 + 六点定案（决策 6 v0.2.3 修订：源码形态可运行 + 打包能力常态化） |
| [02-PRD](./docs/02-PRD-产品需求-Product-Requirements.md) | v0.1.3 | 产品需求 + 业务闭环 + §3.11 对话默认主入口 |
| [03-Architecture](./docs/03-架构设计-Architecture-Design.md) | v0.1.2 | 四层架构 + pi 扩展 + §6.7 会话管理 + §2.3 model_select 落点业务数据根 config/models.json |
| [04-Todo](./docs/04-任务清单-Todo-List.md) | v0.1.86 | 任务登记 + 组件治理看板 + 里程碑 M0-M4 + §6.0 M0 完成说明（v0.2.3 打包能力常态化）+ §6.3 M1 退出门槛全通过 + §6.4 M2 退出门槛全通过 + §6.5 M3 退出门槛全通过 + §6.6 M4 退出门槛（后端断裂修复 + 设置页/学期切换 + S1-S7 接线 + 打包冒烟）+ §7.6.1 M4 任务登记表（T-M4-001~007/T-M4-022 done；T-M4-008~021 pending）+ §7.5 全局执行顺序表（M4 运行时阻塞 T-M4-022 位于 24，T-M4-006~021 顺延至 25-40） |
| [05-ERD](./docs/05-数据模型-ERD-Data-Model.md) | v0.1.1 | 全局库 + 学期库 + 三层记忆 |
| [06-API](./docs/06-API契约-API-Contracts.md) | v0.1.5 | RPC 契约 + 100+ 方法 + 9 Streams + §4 AgentEvent payload 结构化 + modelsConfig.get/set |
| [07-Workflow](./docs/07-工作流-Workflow.md) | v0.1.2 | 学生主路径 + 对话路径 + 11 状态机 + §2.8 工具→Tab 映射表 |
| [08-Test](./docs/08-测试验收-Test-Plan.md) | v0.1.3 | 测试金字塔 + 四层分层 + 安全不变量 + §6 E2E 框架改 vitest + Electron + §4.2 model_select 断言落点业务数据根 |
| [09-UI](./docs/09-使用者介面-UI-Design.md) | v0.1.3 | 三栏布局 + 💬 对话默认 Tab + S1-S7 标签页 + 模型选择持久化业务数据根 |
...
| `.plan/` | ✅ 已就绪 | 任务计划目录（T-M4-007 已完成并保留计划原件作为历史范围与验收证据；当前执行中任务为 T-M4-008，计划 `.plan/T-M4-008-appshell-dataflow.md`） |
| `.record/` | ✅ 已就绪 | 实施记录目录（历史记录已就绪；本任务记录为 `.record/T-M4-007-实施记录.md`） |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v0.1.72 | 2026-08-09 | T-M4-008 本地实现与质量证据同步：AppShell→Tab 统一学术上下文与只读数据生命周期已实现；新增 `useTabData`、renderer 数据流集成测试及请求竞态/卸载保护；Node 24.14.0 下 type-check、102 files/1017 tests、build、smoke 6/6、contract coverage 127/127、安全不变量 6/6、真实 Electron E2E 16 files/117 tests、UUID 7/7、文档治理与 `git diff --check` 均通过。任务保持 in_progress：两名独立审查意见与 Git 收口尚未完成；不新增 API/handler/schema，不启动 T-M4-009~021。依据：AGENTS.md §4.5、§5、§7、§8.4、§11.1/§11.2 + 用户明确授权。 |
