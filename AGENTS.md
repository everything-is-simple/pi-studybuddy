...
| 文档 | 版本 | 权威范围 |
|---|---|---|
| [00-文档索引](./docs/00-文档索引-Index.md) | v0.1.157 | 文档导航 + 门禁 + 版本历史 + 任务启动提示词资产索引 |
...
| [04-Todo](./docs/04-任务清单-Todo-List.md) | v0.1.153 | 任务登记 + 组件治理看板 + 里程碑 M0-M4；T-M4-026 已完成 Git 收口（M4 26 done/0 in_progress），不启动后续任务。 |
...
| [08-Test](./docs/08-测试验收-Test-Plan.md) | v0.1.4 | 测试金字塔 + 四层分层 + 安全不变量 + §6 E2E 框架改 vitest + Electron + §4.2 model_select 断言落点业务数据根 |
...
| `.pi/prompts/*` | ✅ 已创建 | 工作流模板（wr / plan）+ `task-execution/`（标准执行提示词 + T-M4-014~021 受控任务启动提示词；仅参考，不能替代 §4.4 的唯一 `.plan/`） |
...
| `.plan/` | ✅ 已就绪 | 任务计划目录（历史计划保留为证据；当前唯一执行计划为 `.plan/T-M4-026-ai-provider-config.md`） |
| `.record/` | ✅ 已就绪 | 实施记录目录（历史记录已就绪；最新记录为 `.record/T-M4-026-实施记录.md`，记录本地实现、验证、审查与未解决边界） |
