...
**版本**：v0.1.144
**日期**：2026-08-19
**状态**：✅ T-M5-011 执行中。先在当前开发机按真实用户路径跑通，再复制同一套运行资源和默认配置到使用机；系统集成四份权威文件见 `docs/系统集成/`（总览、问题总账、验收与交付路径、固定工作流与阶段门禁）。第四份文件已按用户裁决立即作为执行约束，治理基线审查批准仍待 §11.4 的两名独立审查者交叉核对。（v0.1.143：T-M5-010「设置、投递、学期备份与数据根迁移闭环」已完成 Git 收口；T-M5-005 仍 blocked，仅等待其既定三条干净 Windows 原生 UAT；T-M5-006~008 不启动，当前工作区不读取、不暂存安装器。）
...
5. 系统集成任务必读 docs/系统集成/01-04 ← 客观认知 / 问题总账 / 验收交付 / 固定工作流
6. 相关设计文档（依据任务范围）       ← 01-TRD / 02-PRD / 03-Arch / 05-ERD / 06-API / 07-Workflow / 08-Test / 09-UI
```
...
| [13-测试与运维](./docs/13-测试与运维-Testing-and-Operations.md) | ✅ v0.1.4 已审查批准 | 用户动作、错误责任、数据资产/SQLite 生命周期、真实测试、开发机优先、使用机复制和发布运维门禁。 |
| [系统集成](./docs/系统集成/) | ✅ T-M5-011 执行中；04 待 §11.4 交叉审查 | 后续系统集成唯一依据：01 客观认知、02 问题总账、03 验收与交付路径、04 固定工作流与阶段门禁；每个阶段必须按 04 执行直至 E7。 |
...
| `docs/10-开发规范` | ✅ 已审查批准 v0.1.3 | 任务级 16 步开发流程；系统集成阶段循环由 `docs/系统集成/04-系统集成固定工作流与阶段门禁.md` 补足 |
...
| `.plan/` | ✅ 已就绪 | 任务计划目录；当前唯一执行计划为 `.plan/T-M5-011-system-problem-ledger.md`，系统集成口径统一引用 `docs/系统集成/` |
...
- `docs/10-开发规范` / `docs/11-组件装配` / `docs/12-目录治理`
- `docs/系统集成/01-系统集成总览与客观认知.md` ~ `04-系统集成固定工作流与阶段门禁.md`
...
| 版本 | 日期 | 变更 |
| v0.1.144 | 2026-08-19 | 用户恢复每个任务必须执行固定工作流程，并批准针对系统集成期建立第四份权威文件 `docs/系统集成/04-系统集成固定工作流与阶段门禁.md`。§0 将系统集成任务的 01-04 纳入强制入口；§3.1/§3.3/§11.1 同步四文件地位和任务级 16 步与阶段级 12 步的关系。影响：每个系统集成阶段必须按 04 执行、取证、回退和等待授权；不改变 T-M5-011 in_progress、不进入 S2、不读取/暂存安装器。依据：用户明确裁决 + AGENTS.md §2/§4/§5/§7/§11；04 的治理基线审查仍待 §11.4 两名独立审查者。 |
