...
```text
开发机真实主路径 → 修复并复跑 → 固化运行资源/默认配置 → 复制到使用机 → 最终安装包
```

单元测试、集成测试、Electron E2E 和安全检查继续保留，但服务于主路径定位和回归；它们不能单独把系统标记为可用。所有已知缺口见 [系统集成/02 问题总账](./系统集成/02-系统问题总账与整改清单.md)。
...
| 版本 | 日期 | 变更 | 原因、影响与依据 |
|---|---|---|---|
| v0.1.3 | 2026-08-14 | 同步 T-M5-005 的任务运行状态：该业务 UX 任务已消费 `docs/traceability/` 基线，功能/证据提交 `112da31` 已进入 master 并通过完整自动化门禁；原生 UAT仍仅部分完成，完整报告/投递、成功转写、真实恢复、设置值重启回读、TTS 已复习持久化及独立审查仍未完成。原因：修复“当前无执行中任务”漂移。影响：不改变本文规范、业务 API/schema/handler/入口或数据。依据：AGENTS.md §4.4/§4.5/§7/§11。 |
