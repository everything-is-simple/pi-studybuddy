...
1. `ACT-*` 原子动作和统计口径是否批准；控件、RPC、负向情形是否继续分开统计；
2. 错误分类、错误责任切割和用户恢复条目是否足以成为新的统一标准；
3. 数据资产目录是否应作为 05-ERD 的补充长期维护；
4. 根级 `storage/` 与 `semester/.../storageKey` 的职责核对结果；
5. 发布前运维门禁是否需要进一步细化为独立任务或 runbook；
6. 本文经批准后，哪些任务先补动作目录、错误目录、数据资产目录和真机 UAT。
