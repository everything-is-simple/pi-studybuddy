...
| T-M5-003 | 对话/会话/模型/文件引用真实用户闭环修订 | 壳层/对话 | P0 | in_progress | 阶段2-5 | 07-WF §2.8 + 09-UI §3.3/§4.2/§7-§9 | 用户明确批准开工；唯一计划 `.plan/T-M5-003-chat-session-model-closure.md` 与分支 `agent/T-M5-003-chat-session-model-closure` 已建立；先写 RED，移除 `sess-001`/`mist-001` fixture 语义，不启动 T-M5-004~008 |
