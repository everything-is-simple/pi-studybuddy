...
| T-M4-014 | S4 错题 Tab RPC 接线（list + confirmErrorCause + redo + weakPoints） | 壳层 | P1 | in_progress | 阶段4 | 09-UI §4.7 + 06-API §3.6 | `.plan/T-M4-014-s4-mistakes-rpc.md` + `.record/T-M4-014-实施记录.md`；MistakesTab 已接通 `mistakes.list/get/suggestErrorCause/confirmErrorCause/redo` 与 `weakPoints.list`，局部详情选择、AI“仅供参考”、六分类确认、重做刷新、课程/错题竞态、卸载保护、重复 mutation、归档只读和错误/隐私净化已覆盖；RED 初次 5/5 失败后 GREEN；定向 renderer 14 tests + integration 6 tests、真实 Electron E2E 1 file/1 test、Node24 `verify --stage=full` 通过；无 API/handler/schema/AppShell 全局状态变化；Git 收口待用户单独授权，不启动 T-M4-015~021 |
