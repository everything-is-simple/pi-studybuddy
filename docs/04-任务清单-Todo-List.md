...
**版本**：v0.1.206
**日期**：2026-08-17
**状态**：✅ 已审查批准（M5 进行中：T-M5-001/002/003/004、009/010 已 done；T-M5-005 blocked，其三条遗留原生 UAT并入 T-M5-008 最终干净机安装验收；T-M5-006 实现提交 `5911722` 已推送任务分支，仍为 in_progress，执行必需运行依赖自包含与离线能力装配；真机 UAT 与自动化质量门已通过，双独立审查与受控收尾仍待完成；T-M5-011/007/008 pending 且不启动；当前工作区不读取、不暂存安装器。）
...
| 49 | T-M5-011 | M5 | 本机配置资产与设置能力控制台闭环 | T-M5-006 + T-M5-010 | P0：先完成受管运行能力，再将非秘密配置、DPAPI 资产、七类设置与重启回读收束为单一用户控制台 |
