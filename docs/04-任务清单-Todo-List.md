...
| T-M4-007 | 学期/课程切换 UI（左侧栏学期树 + 标题栏动态绑定） | 壳层 | P0 | in_progress | 阶段4 | 09-UI §3 + 03-Arch §6.7 + 06-API §3.3 | 左栏学期/课程树、唯一上下文、标题、归档只读浏览、按学期隔离的竞态/卸载保护和安全展示已实施；新增记录型 RPC 与 happy-dom AppShell 实挂载测试，完整质量门、UUID、文档治理、diff 检查均通过，Mill/Erdos 两名独立审查最终 PASS；功能提交 `9e5116f` 已在 master 快进合并并以 Node 24.14.0 完整 verify full 复验，正待本次收口推送；不新增 API，不启动 T-M4-008 |
