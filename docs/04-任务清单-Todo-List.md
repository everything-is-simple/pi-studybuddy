...
**版本**：v0.1.186
**日期**：2026-08-14
**状态**：✅ 已审查批准（M5 进行中：T-M5-001/002/003/004、009 已 done；T-M5-005 已完成盘点、RED→GREEN、定向回归与部分隔离原生 Electron UAT，保持 in_progress；S6 完整报告/投递、S7 成功转写、真实备份恢复和设置值重启回读未覆盖。T-M5-006~008 保持 pending；用户已授权当前改动提交、推送和 ff-only 合并，Git 收口执行中。）
...
| T-M5-005 | S6/S7/TTS/备份/设置与整体 UX 修订 | 壳层/S6-S7/跨切 | P0 | in_progress | 阶段1-4，原生 UAT 部分完成 | 07-WF §3-§5 + 09-UI §4.9-§13 + 13-测试与运维 §3-§9 | 用户于 2026-08-14 明确批准开工；唯一计划 `.plan/T-M5-005-s6-s7-tts-backup-settings-ux.md`，分支 `agent/T-M5-005-s6-s7-tts-backup-settings-ux`。盘点后以 RED→GREEN 补足速背卡/转写结果/报告朗读入口并复用 AppShell TTS 所有权，`overwrite` 恢复须先确认；定向 renderer 合同 4/4、相关 RPC 47/47、Electron renderer E2E 7/7、Node24 type-check/build 通过。隔离根真实 Electron UAT 已证实向导创建学期/课程→重启展开/选择回读、冲刺无确认考试中文空态、设置保存反馈、最大化导航、采集合规空态和原生 WAV 选择器；无完整 UUID/绝对路径/API key/错误栈/原始 RPC 异常泄露。尚未覆盖确认考试→速背生成/朗读、报告生成/投递/详情朗读、成功转写/失败恢复、真实备份恢复与设置值重启回读；T-M5-006 依赖自包含边界如实阻塞。`docs/traceability/` 和 `.record/T-M5-005-实施记录.md` 已登记；不写生产数据、不启动 T-M5-006~008，Git 收口另待授权。 |
