...
| T-M3-004 | AI 自主调用工具（S1-S7+TTS+备份恢复全部工具）+ 跳转结构化 Tab | 扩展层 | P1 | in_progress | 阶段3-4 | 07-WF §2.8 + 09-UI §4.2 | 计划 .plan/T-M3-004-ai-tools-jump.md — 五裁决已批准：① 工具→Tab 映射表（35 工具全覆盖 + update_learn_status→notes + backup_* 不渲染跳转按钮留 T-M3-006 + 映射表条款补 07-WF §2.8）② 触发词按域分组（每域 1-2 触发词，保留既有 3 触发词兼容）③ 跳转按钮统一文案 [去<Tab名>] ④ 跳转 context { tabId, sessionId?, courseId? } 脱敏 ⑤ 受控发射扩展测试确定性 |
