...
**用途**：从设计文档到实现代码的执行桥梁——任务登记、组件治理状态跟踪、完成门槛门禁、修复证据记录；v0.1.74 登记 T-M4-022 完成（Electron 生产运行时 / SQLite 兼容修复 + 真实桌面启动验证，前置于 T-M4-006；commit 0ec4163 已推送 origin/master）；v0.1.75 登记 T-M4-006 设置页 UI in_progress（计划待用户审查）；v0.1.78 同步 T-M4-006 双独立复审最终 PASS 与 diff 检查通过，Git 收口未授权故保持 in_progress
...
| T-M4-008 | AppShell 数据流重构（semesterId/courseId 全局状态 + 各 Tab useEffect 拉数据） | 壳层 | P0 | in_progress | 阶段4 | 03-Arch §6.7 + 09-UI §3 | `.plan/T-M4-008-appshell-dataflow.md` + `.record/T-M4-008-实施记录.md`；`useTabData` 统一 idle/loading/error/empty/ready 生命周期；Home/Materials/Notes/Practice/Mistakes/Report 只读列表加载；旧响应/卸载保护；新增 renderer-tab-dataflow 集成测试；Node 24.14.0 verify full 通过；两名独立审查与 Git 收口待完成 |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v0.1.87 | 2026-08-09 | T-M4-008 实现证据同步：统一上下文数据流、只读 Tab 生命周期加载、竞态/卸载保护与测试已落地；Node 24.14.0 完整质量门通过。任务仍 in_progress，独立交叉审查与 Git 收口待完成；统计保持 M4 13 pending / 1 in_progress / 8 done，总计 13 pending / 1 in_progress / 44 done。 |
