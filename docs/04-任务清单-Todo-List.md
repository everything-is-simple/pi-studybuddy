...
|---|---|---|---|---|---|---|---|
| T-M5-001 | 全 UI/功能/依赖用户验收审计与追踪矩阵 | 测试/文档 | P0 | in_progress | 阶段1-5 | 01-TRD §3/§7 + 07-WF §2-§5 + 08-Test §5-§7 + 09-UI 全文 | 逐页逐按钮清单；设计→实现→RPC/本机能力→测试证据；真实安装截图；P0/P1/P2 差异清单；禁止用静态渲染或代表性 RPC 代替 UAT |
| T-M5-002 | 首次启动向导与学期/课程/考试/任务 UI 闭环 | 壳层/S1 | P0 | pending | 阶段2-5 | 07-WF §2.2 + 09-UI §3/§4.3 | 空数据根只用 UI 创建学期、课程、考试、任务/课表；支持编辑/状态迁移/归档前备份；所有入口可发现、可取消、可重试 |
| T-M5-003 | 对话/会话/模型/文件引用真实用户闭环修订 | 壳层/对话 | P0 | pending | 阶段2-5 | 07-WF §2.8 + 09-UI §3.3/§4.2/§7-§9 | 移除 `sess-001`/`mist-001` 等 fixture 语义；新建/切换/重命名/导出/删除；真实模型状态与失败回滚；真实错题/文件选择；流式、工具跳转、朗读可恢复 |
