...
**版本**：v0.1.212
**日期**：2026-08-17
**状态**：✅ 已审查批准（M5 进行中：T-M5-001/002/003/004、009/010 已 done；T-M5-005 blocked，其三条遗留原生 UAT并入 T-M5-008 最终干净机安装验收；T-M5-011 为唯一 in_progress，用户已明确授权其当前分支生成方案 B 测试夹具型 setup；该 setup 只用于测试便利，不等同 T-M5-007/008 发布验收；T-M5-006 blocked 等待独立审查与受控收尾；当前不读取、运行、暂存或删除 `nul` 与 `omp-windows-*.exe`。）
...
- [ ] 本机配置资产（通用偏好、模型选择/目录、凭证引用、运行能力状态）具有明确 owner、介质、原子写/版本迁移、重启回读、备份/恢复和卸载边界；密钥仍只在 DPAPI vault，不进入 SQLite、普通配置、日志或 DOM。
...
> 本表登记任务边界、状态与顺序。按 AGENTS.md §4.4，同一时刻只允许一个 `in_progress` 任务拥有唯一详细计划；其余 pending/blocked 任务不得提前创建详细计划或进入实施。
...
| 49 | T-M5-011 | M5 | 本机配置资产与设置能力控制台闭环 | T-M5-010；T-M5-006 实现验证完成但收尾审查待补 | P0：用户明确批准开工；将非秘密配置、DPAPI 资产、七类设置与重启回读收束为单一用户控制台，006 的运行能力只作只读派生输入 |
| 50 | T-M5-007 | M5 | 发布前真实 Electron 全功能系统 UAT 与回归 | T-M5-002~006 + T-M5-009 + T-M5-010 + T-M5-011 | P0：不生成 setup，先用真实 Electron、隔离空根和纯 UI 完成全控件、配置持久化与故障恢复签收 |
| 51 | T-M5-008 | M5 | 最终 setup/portable 与干净 Windows 安装发行验收 | T-M5-007 + T-M5-009；消费 T-M5-005 遗留断言 | P0：只在发布前 UAT 全通过后生成最终候选，并在干净机完成安装、三条遗留 UAT及最终签收 |
...
| 版本 | 日期 | 变更 |
| v0.1.212 | 2026-08-17 | 按用户命名重新生成方案 B 测试安装包：安装器文件名改为 `pi-studybuddy-test--方案b-setup包.exe`，Windows 产品名、test-bundle manifest 与启动器标题同步为 `方案b-setup包`；SHA-256 `09c5ed2a27b8e4521596e5bc8fe7e3765bfe9fdadfc04e0c40aa71b5f87d6a98`，130418209 bytes；win-unpacked 带 test profile 两次隔离启动与业务 RPC 通过。旧哈希 `4a52a7bf...` 保留为历史证据。 |
| v0.1.211 | 2026-08-17 | 方案 B 测试 setup 已实际生成：新增显式 test profile、正式 handler 合成 fixture、三文件 `test-bundle`、独立 electron-builder 配置、包内容扫描和 package smoke。setup SHA-256 `4a52a7bf2729b3ade6e5eaf15e6f1c7ef03f03814a188c3d5a3b5a840f8f68ae`，130417990 bytes；win-unpacked 双启动、隔离静默安装、安装后双启动通过。Node24 `143 files/1256 tests`、type-check、contract 132/132、安全 6/6、docs/diff 通过；首次扫描和首次安装后 smoke 超时如实保留。T-M5-011 仍 in_progress，不替代原生 UAT/双审查/T-M5-007/008。 |
| v0.1.210 | 2026-08-17 | 用户明确确认方案 B 测试夹具型 setup 生成：T-M5-011 当前计划新增隔离 `test-bundle`、正式 handler 生成的合成 fixture、显式 test profile 启动器、包内容敏感扫描、x64 setup 与 package smoke；不读取/运行/暂存/删除现有安装器，不改变 T-M5-007 发布前 UAT与 T-M5-008 最终候选的顺序和验收含义。依据：用户明确指令 + AGENTS.md §2/§4.4/§4.5/§5/§7/§9/§11。 |
| v0.1.209 | 2026-08-17 | 用户授权复核此前跨任务 Electron E2E 两项失败：Node24 显式排除历史安装器 T-M4-021 后完整运行 `34 files/142 tests` 全绿，T-M5-004 renderer fixture 与 T-M5-006 runtime settings 均通过；未观察到可复现的生产或测试代码缺陷，故不伪造代码修复。T-M5-011 仍为唯一 in_progress，七类原生 UAT、异常恢复、双审查与受控收尾门禁不变；不读取或处理安装器，不自动 Git 收口。依据：用户明确指令 + AGENTS.md §4.5/§5/§7/§11。 |
| v0.1.208 | 2026-08-17 | T-M5-011 实现与阶段性验收事实同步：已完成版本化 config store、普通配置 envelope/迁移/原子写/损坏恢复、DPAPI 固定错误语义、七类设置控制台及 `reportTargets.sendTestMessage` 的固定脱敏测试消息隔离；新增 Renderer 成功/失败反馈断言。Node24 `pnpm test` 142 files/1254 tests、type-check/build、contract 132/132、安全 6/6、`verify --stage=full --skip=e2e`（8执行/2跳过）与专属真实 Electron E2E通过；原生 UAT已覆盖纯 UI 学期/课程创建、通用设置重启回读、SMTP DPAPI 无回显、报告目标重启回读及固定测试消息。未覆盖的五类逐类原生回读和异常恢复、双独立审查与受控收尾仍为门禁，任务保持唯一 in_progress；跨任务 T-M5-004/T-M5-006 E2E失败未修复、不纳入本任务通过结论；不处理安装器或 Git 收口。依据：用户任务指令 + AGENTS.md §4.5/§5/§7/§9/§11。 |
| v0.1.207 | 2026-08-17 | 用户明确批准 T-M5-011「本机配置资产与设置能力控制台闭环」开工：任务 pending→唯一 in_progress，建立唯一计划、隔离分支与运行根，先盘点再写 RED。T-M5-006 的 `5911722` 已推送任务分支、自动化与真机 UAT通过等事实保留；其独立审查、实施记录和受控收尾未完成，故改为 blocked 等待，不代表 done、不合并 master。T-M5-007/008 仍 pending，不读取/暂存安装器。依据：用户明确指令 + AGENTS.md §2/§4.4/§4.5/§5/§7/§8/§11。 |
| v0.1.206 | 2026-08-17 | 用户明确授权 T-M5-006 提交并推送当前任务分支；实现提交 `5911722`（`feat(m5): 装配受管运行依赖与离线降级`）已推送 `origin/agent/T-M5-006-runtime-dependencies`，本地与远端任务分支同位。任务保持 in_progress：未合并 master，双独立审查、实施记录和用户明确受控收尾仍未完成；不启动 T-M5-011/007/008，不读取/暂存安装器。依据：用户明确授权 + AGENTS.md §4.5/§7/§8.3/§8.4。 |
