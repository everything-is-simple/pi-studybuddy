...
| T-M4-010 | S1 首页 Tab RPC 接线（dailyBrief + tasks + exams） | 壳层 | P1 | in_progress | 阶段4 | 09-UI §4.3 + 06-API §3.3 | 本地功能提交 `a06d8a5` 已快进合并进入 `master`，Node 24.14.0 master 完整质量门与独立检查通过；两次 `git push origin master` 因远端连接重置失败，`origin/master` 未核验到新提交，任务保持 in_progress；无 API/handler/schema 变化，不启动 T-M4-011~021 |
