...
| T-M5-009 | 测试与运维追溯基线建立 | 测试/运维/跨切 | P0 | pending | 阶段1-5 | 13-测试与运维 §3-§9 + 08-Test §1/§6/§7 | 建立 `interaction-catalog`（CTRL→ACT）、`error-catalog`（ERR/责任/恢复）、`data-asset-catalog`（DATA/SQLite/文件/配置/生命周期）、`system-traceability`、`operations-runbook` 骨架与 `release-evidence` 模板；只做治理与真实现状盘点，不新增业务 API/schema/handler/入口，不写生产数据，不替 T-M5-005~008 实施业务功能。 |
