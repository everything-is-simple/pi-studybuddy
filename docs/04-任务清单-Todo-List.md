...
| T-M4-010 | S1 首页 Tab RPC 接线（dailyBrief + tasks + exams） | 壳层 | P1 | in_progress | 阶段4 | 09-UI §4.3 + 06-API §3.3 | 本地实施与验收已完成：HomeTab 已接通 `tasks.dailyBrief`、`tasks.list`、`exams.list`，按 semesterId/courseId 分区请求并隔离竞态，保留分域加载/错误/空态；Node 24.14.0 下定向 12/12、相关 14/14、完整 `pnpm verify -- --stage=full` 通过，contract 127/127、安全 6/6、UUID 7/7、文档治理与 `git diff --check` 通过；实施记录 `.record/T-M4-010-实施记录.md` 已建立。任务保持 in_progress，Git commit/merge/push 未授权，不启动 T-M4-011~021 |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v0.1.93 | 2026-08-09 | T-M4-010 本地实施与验收完成，任务保持 in_progress：HomeTab 已接通 `tasks.dailyBrief`、`tasks.list`、`exams.list`，覆盖当前学期/课程上下文、无课程提示、分域加载/错误/空态与课程切换竞态；新增 5 条单元/集成测试，Node 24.14.0 下定向 12/12、相关 14/14，完整 `pnpm verify -- --stage=full` 通过，contract 127/127、安全 6/6、UUID 7/7、文档治理与 `git diff --check` 通过；实施记录 `.record/T-M4-010-实施记录.md` 已建立。无 API/handler/schema 变化；Git commit/merge/push 未授权，不启动 T-M4-011~021。依据：AGENTS.md §4.5、§5、§7、§8.4、§11.2 + 用户 2026-08-09 明确授权。 |
