...
| T-M4-025 | 生产 S2Context 注入真实 TextExtractor（materials.convert 生产可用） | 扩展层/壳层 | P0 | in_progress | 阶段3-4 | 07-WF §2.3 + 06-API §3.4 + 08-Test §3.3.2/§5.4 | `.plan/T-M4-025-s2-extractor-prod.md` + `.record/T-M4-025-实施记录.md`；生产注入 createRealTextExtractor + pdfjs utilityProcess 环境补齐（DOMMatrix shim + pdfjsWorker handler）；materials E2E converted 真实断言；全量 118 files/1130 tests + 28 files/136 tests、verify full 通过；功能 `00b7579` + 治理 `22ecdde` 已快进进入本地 master；origin/master 推送待网络恢复（§8.4 未满足） |
