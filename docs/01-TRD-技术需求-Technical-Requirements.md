...
**版本**：v0.2.2
**日期**：2026-08-07
**状态**：✅ TRD 决策已定案（§7 六点决策经用户明确批准：原五点 + 决策 6 v0.1 交付形态），待 02-PRD 推进（§2.4 会话管理行补"对话默认主入口"语义）
...
> 2026-08-07 用户明确批准决策 1-5。依据：[docs/prep-参考点核对表.md](./prep-参考点核对表.md) 四参考仓库核对结论。决策 6 于 2026-08-07 追加（v0.1 交付形态）。
...
| 6 | v0.1 交付形态 | **源码形态（pnpm dev 直接运行），不打包 .exe** | v0.1 明确禁用"运行级使用"（仅 verification_only / research_only，AGENTS.md §1.2），需可审计；单人单机自用无需分发；治理体系（AGENTS.md + .plan/ + .record/）依赖源码可读性。asar 打包、混淆、自动拉取 venv/whisper 等产品化机制延迟到 v0.2+ 届时再议 |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v0.2.2 | 2026-08-07 | §7 加决策 6「v0.1 交付形态：源码形态（pnpm dev），不打包 .exe」——依据 AGENTS.md §1.2（v0.1 禁用运行级使用）+ §6.4（禁止提前设计 v0.2+ 产品化机制）；M0 完成后在 04-Todo §6.0 补版本演进说明 |
