...
- 2026-08-15：新安装反馈复核并完成设置页模型目录修订。运行时 `pi-models.json` 现对既有 provider 保留用户连接定义且按 model ID 合并内置目录，修复旧安装空数组导致 DeepSeek Flash/Pro 与 Agnes 文本模型不可见；中转站仍须用各自已保存 API Key 执行“获取模型目录”。设置页 API Key 标签及保存状态绑定当前供应商，测试按钮以当前选中 `{provider, model}` 发送最小“连接测试”请求；Key 保存不再被表述为远端已连通。备份目录继续仅在备份 Tab 通过原生目录选择器选择且不入 DOM/设置文件；开发者工具区为诊断，应用无受控下载源时不显示无效安装动作。定向 settings 14/14、catalog/model handlers 16/16、真实 Electron settings 1/1、Node24 type-check 通过。真实安装版仍须确认所运行 setup 包包含本次构建，任务保持 `in_progress`。
