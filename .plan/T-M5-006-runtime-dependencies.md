...
| Git | 用户已授权本次受控改动提交并推送当前任务分支；实现提交 `5911722` 已推送 `origin/agent/T-M5-006-runtime-dependencies` 并核验同位；不得因此执行 master 合并、生成 setup 或提前结束 T-M5-006 | 已完成当前授权；任务仍 in_progress |
...
完成本地实施不等于任务 done。仅当五阶段、完整质量门、真机 UAT、两名独立审查、`docs/04`、实施记录和计划状态均完成时，才能申请收尾。用户已授权的任务分支提交/推送已完成：`5911722` 已推送并核验同位；不得因此执行 master ff-only 合并、生成 setup 或提前结束 T-M5-006。后续功能改动仍须按各自测试和收尾门禁处理。
