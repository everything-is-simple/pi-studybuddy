...
**版本**：v0.1.3（T-M5-011：开发机优先真实闭环整改）
**日期**：2026-08-19
...
## 文档体系（01-13 ✅ 已审查批准；系统集成 01-04 执行中）
...
| 13 | [测试与运维](./docs/13-测试与运维-Testing-and-Operations.md) | ✅ v0.1.4 已审查批准 | 用户动作、错误归属、数据资产/SQLite 生命周期、真实测试、真机 UAT 与发布运维门禁 |
| 系统集成 | [01 总览](./docs/系统集成/01-系统集成总览与客观认知.md) / [02 问题总账](./docs/系统集成/02-系统问题总账与整改清单.md) / [03 验收交付](./docs/系统集成/03-系统集成验收与交付路径.md) / [04 固定工作流](./docs/系统集成/04-系统集成固定工作流与阶段门禁.md) | 01-03 ✅ v0.1.2；04 ✅ 用户裁决已登记、待 §11.4 交叉审查 | 当前系统集成唯一依据；04 的阶段级 12 步工作流自 T-M5-011 执行至 E7 |
...
当前优先目标不是让使用者配置开发环境，而是先在当前开发机把真实系统跑通。开发机使用固定的 Electron/Node、Python/OCR、whisper 和 WPS 运行资源；闭环稳定后，再将同一套资源和启动拓扑复制到使用机。

普通使用者不应安装或配置 Node.js、pnpm、Python、Git 或 OCR venv。旧版 DOC/PPT/XLS 通过 WPS 转换，但 WPS 必须先在开发机完成真实主路径验证后才能进入复制交付。

### 当前阶段（T-M5-011：系统问题总账与整改）
...
│   ├── 13-测试与运维.md          # ✅ v0.1.4 已审查批准
│   └── 系统集成/                  # 当前系统集成四份权威文件
│       ├── 01-系统集成总览与客观认知.md
│       ├── 02-系统问题总账与整改清单.md
│       ├── 03-系统集成验收与交付路径.md
│       └── 04-系统集成固定工作流与阶段门禁.md
│
├── .pi/                         # pi 生态治理资产（✅ 已就绪）
│   ├── skills/                  # 治理用 Skill
...
所有影响“系统正常运行”的已知问题、阶段执行和交付判断，统一以 [系统集成四份文件](./docs/系统集成/) 为准：

1. [01-系统集成总览与客观认知](./docs/系统集成/01-系统集成总览与客观认知.md)：客观判断模型、能力边界和 E0-E7 证据等级。
2. [02-系统问题总账与整改清单](./docs/系统集成/02-系统问题总账与整改清单.md)：问题、根因、责任、证据和整改顺序的唯一总账。
3. [03-系统集成验收与交付路径](./docs/系统集成/03-系统集成验收与交付路径.md)：开发机、使用机和安装包的验收顺序及交付判据。
4. [04-系统集成固定工作流与阶段门禁](./docs/系统集成/04-系统集成固定工作流与阶段门禁.md)：每个系统集成阶段必须执行的 12 步工作流、真实 UI/数据/重启回读、问题回填、质量门和停止授权；该文件已按用户裁决立即执行，治理基线审查批准仍待 `AGENTS.md §11.4` 的两名独立审查者交叉核对。

`docs/10-开发规范` 的任务级 16 步流程仍然有效；系统集成期还必须执行第四份文件的阶段级 12 步循环，直至安装包 E7。当前按“开发机真实闭环 → 固化运行资源 → 复制到使用机 → 重建安装包”推进。
...
8. **系统集成四文件 + 固定阶段工作流** — 当前系统集成阶段必须读取 01-04，逐阶段执行第四份文件的 12 步门禁
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v0.1.3 | 2026-08-19 | 用户恢复每个任务必须执行固定工作流程；README 明确系统集成四份权威文件的分工，以及任务级 16 步与系统集成阶段级 12 步的并行约束。第四份文件按用户裁决立即执行，治理基线审查仍待 AGENTS.md §11.4 两名独立审查。 |
